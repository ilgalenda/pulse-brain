...
brain. It is not a new operation surface replacing the CLI, but the shape of the
...
needs a live instance or a later sync backend. It introduces **no new kernel
...
- **Output (deliverables):** how the brain presents what it hands back, such as
  briefings, reports, and exported documents. **Defined now as a standard** (§5).

The principle: the layer is **designed in full, built only where building is honest**.
A UX layer's worth lives in the lived experience, which arrives once you instantiate
your brain, so the experiential parts are specified now and matured then, not
gold-plated against a vault that does not yet exist.
...
2. **Wakes the brain:** runs read-routing ([kernel §7](OPERATIONS-KERNEL.md)),
   loading core memory (`active_preset`, current focus, live threads, the Needs-review
   flags) and `index.md`. This *is* the kernel's session-start read, made explicit.
3. **Greets and reports state:** `Hi {{OWNER}}`, then a compact status of active
   preset, current focus, items needing review, and last activity.
...
(composing [`/preset`](vault-template/.claude/commands/preset.md)). It is gated by the
...
The Inbox pillar accepts material from whatever channel fits, with **no bespoke app
required**. The channels, by reach:

- **CLI capture:** the desk channel, which already exists (the ingestion front-door,
  [kernel §1](OPERATIONS-KERNEL.md); the Inbox pillar, [CONTEXT-ENGINE](CONTEXT-ENGINE.md)).
- **Remote Control:** operate and capture from the phone **with the Mac awake**. It runs
...
- **Synced drop-folder:** the **async** channel for when the Mac is asleep, a watched
  folder (iCloud, Dropbox, or Obsidian Sync) the Inbox ingests when next awake. The
...
`index.md` is the map-of-content ([kernel §6](OPERATIONS-KERNEL.md)), and `/pulse`
...
mature with the live vault rather than being designed blind now.
...
- Templated scaffolding is absent. There is no throat-clearing opener, no symmetrical
  paragraph shape, and no closing flourish that restates without adding.
...
So that output sounds like {{OWNER}} rather than a generic register, setup
...
- **Generic and placeholdered.** The banner and command name no real person; `Hi
  {{OWNER}}` fills at instantiation, like every placeholder. No live data.
...
- **Local-only phone access.** Remote Control keeps files on the machine (only prompts
  and outputs transit, encrypted), while **Claude Code on the web** clones the vault into an
...
- **To the Operations Kernel:** every surface composes `load` and the ingestion
  front-door; `/pulse` is read-routing ([§7](OPERATIONS-KERNEL.md)) made explicit. No
  primitive.
- **To the Context Engine and Inbox:** the capture channels feed the Inbox pillar; this
  layer routes *into* it, it does not redefine it.
- **With Persona and Presets:** `/pulse` reports the `active_preset` and can wake into one
  (composing [`/preset`](vault-template/.claude/commands/preset.md)).
- **To MCP hosting:** the hosted capture endpoint and external MCP surface.
- **To instantiation:** the experiential UX (review dashboards, boot polish, the
  drop-folder ingest) realises and matures against the live vault; setup also runs the
  owner-voice ask (§5.4).
- **With the global writing-craft standard:** the output standard (§5.1) inherits it; the
  brain's deliverables hold to British English and a genuine, non-templated voice.
- **To tooling and substrate:** the PDF renderer (§5.3) is deferred code, not built in
  this lean pass.
...
> left off. The deliverable standard (§5) sets how it presents what it hands back:
...
> realise progressively, as real use and the live vault make them honest to build.
