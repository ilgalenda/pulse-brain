...
LLM follows, and what it produces *may itself be code* (a tool). An earlier
foundation shipped a **seed**: three `create-*` wrappers, the `instruments.md`
registry, and the `agent-architecture-audit` QA. This document is the
**authority for the layer**; those seeds, generalised here, are its executors.
...
**Self-Development & Growth** meta-loop ([BRAIN-OS](BRAIN-OS.md)), which audits the
brain for gaps and *drives* Builder, Research, and Thinking toward system-wide
growth. The line is sharp:

- **Builder:** "this task needs a capability that isn't here, build it now."
- **Self-Development:** "the brain is weak *here*, Builder, go build."

Builder is the *hands*; Self-Development is the *judgement that points them*.
Keeping them separate means the act of building is identical whether {{OWNER}}
asks or the meta-loop asks: one lifecycle, two callers.
...
- **Composes** the kernel **capability contract**
  ...
  `ingest`/`integrate`/`load`/`flag`/`update-*`, and Builder **adds no new primitive**.
...
- **Does not** own the meta-loop (Self-Development & Growth), the kernel contract
  (it composes it), the page editorial standard (Context Engine), the graph model,
  or the heavy code substrate. MCP hosting and embeddings/ML belong to the model
  and retrieval substrate, not Builder.
...
| **Agent** | An autonomous *worker* (own context plus least-privilege tools) that runs skills | Instruction | `.claude/agents/<name>.md` |
...
gathering is an *agent's* job under injection defence (§6, [Research & Background
Agents](RESEARCH-AGENTS.md)).

## 4. The build lifecycle

Every build, whichever kind and whoever the caller, runs the same disciplined cycle.
...
- **Choose kind:** skill, agent, command, or tool, by the §3 split.
- **Scaffold:** delegate file generation to the global `create` skill, and never
  hand-roll structure.
...
- **Audit:** run [`agent-architecture-audit`](vault-template/.claude/skills/agent-architecture-audit/SKILL.md),
  resolving every Critical/High finding **before** the instrument is trusted.
...
## 5. The reactive trigger and the dual-role gate
...
- **Pulse decides** whether to build. **Build-then-record, not ask-then-build:** a
  new instrument is **local, sanitised, and reversible** (a markdown or script file
  in `vault-template/.claude/`), so it is *not* the "consequential outward action"
  the no-silent-execution rule gates. Builder may proceed and **record** rather than
...
- **QA:** `agent-architecture-audit`, the gate every build clears.
- **Acceptance bar:** the [build-request contract](vault-template/.claude/tests/build-request-contract.md).

## 8. Seams

- **← Builder seed.** The three `create-*` skills, the registry, and the
...
- **→ Operations Kernel.** A built instrument composes the capability contract,
  and Builder adds no primitive.
- **→ `agent-architecture-audit`.** The QA gate (§4); no build is trusted until it
  clears Critical/High.
- **↕ Research & Background Agents.** A new worker (research, monitor, or
  beyond) is created *through* this layer, against the same pattern; the seed
  established the foundation, and Builder generalises it.
- **→ Self-Development & Growth.** This meta-loop *drives* Builder on graph-health
  and gap signals; it decides when and what to grow, and Builder builds it. Builder
...
> roadmap is **Self-Development & Growth**, the meta-loop that decides *when*
