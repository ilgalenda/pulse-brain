...
(`sessions.md`) and one tool (`session-reconcile`). It is the formal protocol the
kernel's *safe-by-default* concurrency posture ([§10](OPERATIONS-KERNEL.md)) and the
Research layer ([RESEARCH-AGENTS §5](RESEARCH-AGENTS.md)) have deferred to S15.
...
Genuine *prevention* — a tab acquiring a lock before it writes and another backing
...
- **References** — the Research layer's safe-by-default posture
  ([RESEARCH-AGENTS §5](RESEARCH-AGENTS.md)) it formalises, and `graph-health-scan`
...
axis the Immutability principle ([PRINCIPLES.md](PRINCIPLES.md)) and S13's autonomy
...
from S5, why a dynamic-note is always a new file, and why `sessions.md` (§5) is
...
Builder layer as `graph-health-scan` did at S13. Its single job is **keep concurrent
...
- **Develop the engine section by section.** The engine is a git repo; brain
  ...
  pressure-tested before commit, **one `S<n>:` commit per section**, linear history,
...
write-class protocol (§3–§6) is the safety half. Together they let the brain be built
...
- **Triggers are instance wiring.** The `SessionEnd` hook and the ~15' cron live in the
  ...
  set at instantiation (S19). Nothing runs on a backend in the engine — there is no
...
- **↔ Persona / Presets** — concurrent tabs may each run a different preset over the
  ...
  question [PERSONA-PRESETS §9](PERSONA-PRESETS.md) handed to S15 — coordinating on
  `core.md` without colliding — is answered here.
...
> reconciler reaps dead sessions and surfaces drift — on clean exit instantly, on a
> loose periodic backstop for crashes. It heals rather than locks; true prevention is
> the named broad-scope seam. Next on the roadmap is **S16 — Interface**.
