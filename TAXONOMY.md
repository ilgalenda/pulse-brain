...
A Pulse brain has three pillars. **`canon/` is the living wiki**; `inbox/` and `dynamic/` are the **raw sources** that are compiled into it (the *compile-don't-retrieve* model — see [`OPERATIONS-KERNEL.md`](OPERATIONS-KERNEL.md)).
...
`inbox/` and `dynamic/` are raw inputs split by origin; `canon/` is the compiled synthesis they feed. There is **no review queue and no pre-write confirmation gate** — this is a single-owner brain. The LLM maintains the wiki autonomously and **surfaces contradictions and significant revisions** for {{OWNER}} to review live in Obsidian (flagged on the page and listed in core memory).

## Domains

Each pillar is organised by **domains** — top-level areas of {{OWNER}}'s work. **Domains are owner-defined**: chosen at instantiation (`/setup`), scaffolded by the `instantiate` tool, and freely changed later. The set below is the shipped **example** — modelled on one owner's dual role (GTM lead + AI orchestrator); a new instance replaces it with its own:
...
`entities/` exists only under `canon/` — entity pages are part of the wiki, not raw sources. The other six domains mirror across all three pillars.
...
- **`id`:** a stable kebab-case slug, set once and not changed — the durable handle for cross-references.
