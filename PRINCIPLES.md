...
Code — and they **sharpen** the existing standards there; they do not replace them.
...
Adopted after S11, where they first governed a build end-to-end. They apply
...
**one job and hands off**; compose specialists rather than build monoliths; run
independent work as **parallel** specialists. This is the director's "use
...
no runaway loops). Sharpens the director's *Security protocols* and the
...
append/supersede, never edited in place. Knowledge changes by **new nodes + explicit,
logged transitions** (e.g. an inference `unverified → weakly-supported → confirmed`,
