...
It is an **instruction** layer (Brain-OS instruction-vs-code map: Thinking =
instruction) — markdown the LLM follows; Claude Code is the runtime. It is the
home of Law 3 (the intelligence partner): the brain that thinks *with* {{OWNER}},
not just *for* them. The graph model it applies is [`NODE-GRAPH.md`](NODE-GRAPH.md);
the runtime it composes is [`OPERATIONS-KERNEL.md`](OPERATIONS-KERNEL.md).
...
pages depend on) is surfaced via the existing flag (§6) — S10 reuses that test,
it does not coin a new threshold.
...
ratify. Unresolved contradictions are also a graph-health signal that
**Self-Development (S13)** monitors (Node-Graph §6/§8) — Thinking supplies the
reasoning, S13 owns the system-level watch.

### 3.4 Content-promotion candidates — *owned (content); feeds (ontology)*
...
- **Edge-type / ontology promotion** — naming a *new typed edge* when a relation
  ...
  *decision* — extending the edge vocabulary — is **S13's**, reviewed through the
  graph-health / Self-Development loop (Node-Graph §3.3/§8). *Feeds, does not own.*

### 3.5 Challenge & advise — *owned (zero seam-overlap)*
...
challenge-&-advise when {{OWNER}} wants the brain's view; socratic when they want to
...
S10 states the stance and stops short of scripting a voice the engine has no data
to ground.
...
owned by **Self-Development (S13)**. (The unsourced-and-unmarked check above is the
one signal Thinking feeds S13's §6 list — see §6.2.) Same `load` primitive,
different object.
...
**Autonomy is S11's, not S10's.** Thinking defines the *capability*; the
**scheduling and background execution** of any trigger — running a sweep
unattended, watching for the moment to be proactive — belongs to **Research &
Background Agents (S11)**. S10 is what S11 runs; S11 is when it runs.
...
**writes** through the same routed, index-keeping path every other write uses —
so the maps never drift (kernel §8). "The runtime everything plugs into" includes
Thinking: it is a consumer of the contract, not an exception to it.

## 6. Grounding & the quality bar
...
### 6.2 Resolution & the missed-marker backstop
...
[Node-Graph §3.2](NODE-GRAPH.md) — no new frontmatter field) and **indexed by its
`basis` pages**. So the brain can find every open inference, and every inference
...
inference is **reasoning, not a mechanical attach** (until [S17](NODE-GRAPH.md §9)
adds semantic matching). The two paths, in priority order:

1. **Active hunt (primary).** §6.3's S11 agents and the sweep walk the
...
### 6.3 The self-correction loop (seam to S11)
...
queue of them. **Research & Background Agents (S11)** walk that set, try to confirm
each in the open world, and resolution clears them per §6.2 — the **primary**
resolution path. This closes Law 1's *"synthesise → (maybe) research"* loop on
itself: the brain checks its own homework without being asked.
...
the existing two-part flag** — the page callout + the "Needs review" list in
...
the pre-write gate S3/S4 abolished (kernel §5; ARCHITECTURE §1 boundary). The
existing flag *is* the surface. Precision over recall — when unsure, the brain
marks rather than asserts.

## 7. Seams to neighbouring layers

- **← Node-Graph (S9)** — Thinking *applies* the model: it traverses edges,
  proposes new ones (§3.2), and respects the seeded-but-open edge vocabulary. It
  reasons over the graph; it does not redefine it.
- **→ Research & Background Agents (S11)** — when reasoning hits a gap internal
  knowledge cannot fill (an open inference, an unsupported claim), Thinking hands
  a research prompt to S11 (§6.3). S11 also *schedules* Thinking's triggers (§4).
- **↔ Self-Development & Growth (S13)** — the split is **content vs system**.
  Thinking reasons over *knowledge*; S13 audits the *platform* — graph health
  (Node-Graph §6), ontology promotions (Node-Graph §3.3), capability gaps.
  Thinking supplies reasoning and surfaces candidates; S13 owns the meta-loop.
...
> **S11 — Research & Background Agents**, the autonomy that runs Thinking
