...
process. It introduces **no new kernel primitive**, because switching a preset is
updating the **`active_preset`** anchor the kernel already reads ([OPERATIONS-KERNEL §7/§10](OPERATIONS-KERNEL.md)).
...
- **Defers** background and scheduled behaviour to the background-agents layer, and
  capability growth to the build-and-grow layers. A preset only *foregrounds* existing
  instruments; it never builds or runs one.
...
- **Tone**: the role's **register and emphasis**, meaning what to foreground, how much
  to challenge, the working vocabulary. **Not a different voice or personality**, since
  there is one voice to {{OWNER}} (§7).
- **Lead instruments**: which agents and skills are foregrounded for the role (e.g. the
  research workers for a sales role, the Builder and audit instruments for an engineering
...
Switching is deliberately lightweight, config rather than machinery:
...
4. **A switch is a logged setting change**, not a knowledge edit; it is recorded like
   any `core.md` update (kernel §10) and touches no canon page.
...
- **A preset cannot hide a flag.** Contradictions and review items in `core.md` are
  always-loaded regardless of preset (kernel §10), so the brain never goes blind to its
...
## 8. Security and the engine/instance boundary
...
- **Generic config, no live data.** A preset names *domains, instruments, tone, and
  ...
  reads. `{{OWNER}}` and `{{COMPANY}}` placeholders throughout.
- **The engine ships exemplars.** `gtm-lead` and `ai-orchestrator` ship as the two
  ...
  matures at instance time against real data, like the Thinking voice.
- **A switch is reversible and logged**, a setting change rather than a content
  mutation. Immutability is untouched: no provenance tier is rewritten, only the
  `active_preset` anchor is set (recorded in `log.md`, kernel §8).
...
- **→ the instruments**: `lead_instruments` foregrounds existing skills and
  agents; a preset never builds, grants, or runs one.
- **↔ Self-Development**: the self-development layer may *notice* that a recurring need
  is unmet under a role and drive Builder to fill it, or surface that a new preset is
  warranted; the preset itself stays config. This layer is the lens, self-development is
  the growth that may reshape it.
- **→ Development**: concurrent tabs may each run under a different preset over
  the one shared vault; how sessions coordinate without colliding on `core.md` belongs
  to the Development layer ([`DEVELOPMENT.md`](DEVELOPMENT.md)), where the `active_preset`
  anchor is a derivable cache field the kernel reconciles from the log.
- **Identity (cross-cutting)**: every preset sits beneath the project director and the
  dual-role and inherits both (§7).

> **The Persona / Presets layer is now in place.** The brain operates as the role
> {{OWNER}} is in (GTM Lead, AI Orchestrator, or another), reading the same knowledge
> through a role-shaped lens of focus, domains, tone, and lead instruments, switchable
> in one command and never at the cost of the knowledge or the one voice beneath it.
> Next on the roadmap is the **Development** layer: concurrent multi-tab operation over
> the shared vault.
