...
- **Layered, by design.** A kernel, a context engine, and active layers for thinking,
  research, building, and self-development sit under a UX and a model and retrieval
  substrate. The whole design is in [`ARCHITECTURE.md`](ARCHITECTURE.md).

## Engine and instance: the data boundary
...
Built one piece at a time, each planned before it was implemented (see
[`ARCHITECTURE.md`](ARCHITECTURE.md) for the design and the roadmap). The architecture is
in place: all ten layers, the instantiation kit, and the data boundary.
