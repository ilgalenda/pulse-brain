...
The brain develops by **growing and refining its node-graph**. The edges between sources, pages, entities, and pillars are where synthesis lives. This is relational learning: the **frontier model stays pluggable** (Claude now, others later), and what grows and is owned is the graph.
...
2. **Capture a source note** → `inbox/<domain>/` (provided by {{OWNER}}) or `dynamic/<domain>/` (found by an agent), using the source template: raw input reference, extracted key points, and provenance.
3. **Integrate**: run the integration pipeline (§3).
4. **Report**: tell {{OWNER}} what changed in the wiki (pages created or revised, contradictions flagged).

Manual filing into a pillar is a fallback, never the requirement. The {{OWNER}}-side capture ergonomics (sub-kinds, source modalities including image and video, and the capture channels) are specified in [`CONTEXT-ENGINE.md`](CONTEXT-ENGINE.md) (Inbox); this section owns the runtime front-door.
...
- carries the atomic-note frontmatter, `## Summary`, and body (per `TAXONOMY.md`),
- **cites the sources** it was compiled from (`[[links]]` back to `inbox`/`dynamic`),
- is interlinked with related pages via `[[wikilinks]]`.

The editorial standard for these pages (what each kind is for, atomicity and page boundaries, how to revise, lifecycle, and the quality bar) is [`CONTEXT-ENGINE.md`](CONTEXT-ENGINE.md) (Canon). This section owns the runtime *shape*; that doc owns *how the page is written*.

## 5. Contradiction & review surface

The LLM maintains freely, but it must make disagreement **visible** and never silently overwrite a claim:
...
- **`index.md`**: the master **content catalogue / map-of-content**, holding the domains, the key entry pages, and links into each pillar's `_index.md`. The orientation map, read at session start.
- **`log.md`**: the chronological **evolution log**, an append-only record of every meaningful change (source ingested, page created or revised, contradiction flagged or resolved). The brain's changelog, and the raw material the Self-Development layer reviews.
...
1. Read **core memory** (anchors: current focus, active preset) and **`index.md`** (orientation).
2. Read `canon/_index.md` (**always**); load the **pinned wiki core** plus the **active domain(s)' relevant** pages, never the whole wiki. The active domain(s) are the task's domain when it names one, else the active preset's `default_domains` ([`PERSONA-PRESETS.md`](PERSONA-PRESETS.md), a set, biased not exclusive).
3. **Identify anchor nodes** for the task (the entities or pages it concerns).
4. **Traverse the graph** from anchors over the index `links`, with **bounded depth**, **distance decay**, and **hub weighting**.
5. Use **domain / tags / recency** as secondary tie-breakers; semantic similarity is added by the Model & Retrieval Substrate ([`MODEL-RETRIEVAL.md`](MODEL-RETRIEVAL.md)) as a **re-rank of this candidate set**. It re-orders the pages the walk already surfaced, never adds pages the walk missed (compile-don't-retrieve, §5). It is built, and turned on in read-routing once threshold calibration is done.
...
- **canon-always-first ≠ load the whole wiki**: always the canon *index*, pinned *core* pages, and *active-domain relevant* pages only.
...
- **Core memory** (`memory/core.md`, **always loaded**): identity / active-preset anchor, current focus, live threads, recent decisions, the **review-flags** list, last-updated. Compact by design, with a hard size ceiling; overflow demotes to the timeline.
...
The kernel is **model-agnostic**. A route or agent **may declare** which model serves it; absent a declaration, the **session model** is used. Models are pluggable, orchestratable resources. This is the hook the **Model & Retrieval Substrate** ([`MODEL-RETRIEVAL.md`](MODEL-RETRIEVAL.md)) realises: the `resolve-model` tool turns a route or agent's declaration into a deterministic lookup against the instance bindings, and **MCP hosting** slots in the same way. Pulse is, in effect, an LLM that grows alongside Claude, and other API models if added.
...
**Skills, agents, commands, and tools are built against this contract** to grow the brain. This is Claude's coding role, and the Builder layer. A capability composes these primitives rather than reinventing them; this is what "the runtime everything plugs into" means. The contract is defined here as instruction; capabilities are built on top later. The **optional small determinism helpers for index integrity** noted in the instruction-vs-code map ([BRAIN-OS](BRAIN-OS.md)) are built as **Builder *tools*** ([`BUILDER.md`](BUILDER.md) §3) that *compose* `update-index`/`update-log` deterministically: the kernel owns the contract; the tool is the helper that supports it.
...
This is the brain's analogue to model development: **relational** learning, with the frontier model pluggable. **Graph health** (connectedness, orphan pages, stale or contradictory edges) is a signal the Self-Development layer monitors and acts on. The owned graph is the moat; the model is rented. The full graph model (node kinds, the seeded-but-open edge vocabulary, traversal, health signals, and growth rules) is the authority in [`NODE-GRAPH.md`](NODE-GRAPH.md); this section is the kernel's edge-work mandate within it.
