...
Brain-OS **layer 10**: the **ML engine room** beneath the kernel, holding the *models the
...
- **the semantic tie-breaker slot** ([OPERATIONS-KERNEL §7.5](OPERATIONS-KERNEL.md),
  ...
  **re-ranking**, and adds the **semantic/vector edges** the Node-Graph seam
  ([NODE-GRAPH §4/§9](NODE-GRAPH.md)) names as belonging to this substrate.
...
re-deriving it per query. This substrate adds *retrieval machinery*, so the boundary must be
...
- **Similarity edges are candidates.** Embedding-derived edges are surfaced for the
  ...
  ([NODE-GRAPH §3.3](NODE-GRAPH.md)). They are **never auto-written** into canon.
...
the Builder or a future orchestrator, passes the session model or active preset *in*.
...
The role-to-model bindings are **instance config**, like the watchlist
...
template**; the populated bindings are stamped when you instantiate your brain. The binding
*rule*, meaning what `model:` a new instrument receives, lives in
...
- **Multi-provider chat plumbing** (calling non-Claude chat providers) stays deferred;
  the brain already runs on the session model, and this substrate formalises *selection*, not new
  chat network calls. Exposing Pulse as an external MCP server is a later layer (MCP hosting).
- **A self-tuning routing loop is not built.** Per-route cost/rework telemetry is **not
  ...
  is itself a tier-3 propose-then-ratify act ([SELF-DEVELOPMENT §6](SELF-DEVELOPMENT.md)),
...
Built **lean and design-first**, the same discipline the interface layer used: the parts whose behaviour
is a function of the live content distribution defer to the live vault (built when you instantiate your brain), because
...
### 3.3 Concurrency: a third integrity lane the development layer does not cover
...
of any whole-store artefact. Two tabs reindexing at once cannot tear the store.
...
("semantic duplicates wait for the Model & Retrieval Substrate") and [NODE-GRAPH §6](NODE-GRAPH.md) ("duplicate
...
- **Thresholds are instance config (set when you instantiate your brain).** What cosine distance counts as *relevant* or
  *duplicate* can only be calibrated against real content; a cold-hardcoded cut is the
  ...
  an explicit calibration step gates "trust the output".
...
([BUILDER §3](BUILDER.md): deterministic, no network, no credentials). So this layer introduces
...
- **Deferred to when you instantiate your brain (needs the live content distribution):** threshold calibration; real
...
- **Deferred and named (not built):** any metered routing-telemetry signal and the
  self-tuning loop over it (needs a tier-3 Node-Graph ontology addition first); MCP hosting.
