<!--
TEMPLATE — the inbox ({{OWNER}}'s sources) edge-map. The kernel maintains this;
...
The capture contract, sub-kinds, and modalities for these sources are in
CONTEXT-ENGINE.md (Inbox). Inbox sources are implicitly trusted — unlike the
dynamic edge-map, there is no confidence/trust tag convention here.
The node/edge model the `links` column serialises is NODE-GRAPH.md.
-->

# Inbox index ({{OWNER}} sources edge-map)
