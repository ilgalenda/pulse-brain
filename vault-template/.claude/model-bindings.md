# Model bindings: role to model (INSTANCE CONFIG)

The brain's **model-routing policy**: which model each *role* runs on. The deterministic
[`resolve-model`](tools/resolve-model) tool reads this file, and the Builder consults it when
...
> **This is INSTANCE config.** The engine ships it commented and placeholdered, never
> populated. Concrete model ids are filled in when you instantiate your brain, alongside the
> watchlist and presets. A role with no binding falls back to the **session model** (the kernel
...
- **Main loop, strongest model (recommended: Opus).** Owns integration, synthesis,
  partner judgement, and the canon and log writes, which are kernel-owned, non-delegable, and the
  highest stakes. Set via `/model`; not bound here.
- **Worker roles, tiered by the task:**

```
# --- worker bindings (uncomment + set concrete model ids when you instantiate your brain) ---
