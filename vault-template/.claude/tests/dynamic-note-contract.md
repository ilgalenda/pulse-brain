...
- [ ] `## Summary`: one paragraph saying what the source says.
- [ ] `## Extracted points`: the specific, integrable claims (the payload). A
      source that yields **no** extractable point is not worth capturing, so reject it.
- [ ] `## Provenance`: where it came from, when, and how it was retrieved, enough to re-find or cite.

## Discipline checks

- [ ] **Atomic**: one source, one note. A find spanning two distinct subjects is two notes.
- [ ] **Not the answer surface**: the note is raw provenance, and the *wiki* is the answer. It does not pretend to be canon.
...
- [ ] **Supersession is represented correctly**: when a new note replaces a stale one, the link is recorded per [NODE-GRAPH §3.2](../../../NODE-GRAPH.md), namely a `related` link to the replacement plus a one-line body note, surfaced as an `edge/supersedes` tag. There is **no dedicated `supersedes` frontmatter field**, since the supersession model deliberately adds no new schema field. The superseded note is kept for provenance, not deleted.
- [ ] **Deduped**: a search for the same `source` found no existing note; if one existed, the instrument refreshed or superseded rather than duplicated.
- [ ] **No live secrets**: no credentials or tokens embedded, and auth (if any) referenced via `.env`.
...
*Why `confidence: medium`:* single-source, vendor-authored (an interested party),
no corroboration, so rated down per the rubric.
