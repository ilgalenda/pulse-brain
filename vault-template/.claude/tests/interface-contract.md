...
boot/entry command [`/pulse`](../commands/pulse.md). It has two halves. Part A checks
that the boot artifacts are well-formed (banner byte-exact, generic, placeholdered).
Part B checks that `/pulse` boots correctly: it paints the banner, wakes the brain via
the kernel's read-routing, reports state, and composes `load` without a new primitive.
This restates INTERFACE §2 as a *test*, not a new authority. If any item fails, the
surface is not yet correct.
...
- [ ] **Generic and placeholdered.** The greeting is `Hi {{OWNER}}` (filled when you
      instantiate your brain); no real name, account, or live data appears anywhere in
      the banner or command.
...
- [ ] **Paints then wakes.** It `cat`s the banner, then runs read-routing
      ([kernel §7](../../../OPERATIONS-KERNEL.md)): it reads `memory/core.md` and
...
- [ ] **Reports true state.** The status block reflects the *actual* loaded state.
      `preset` is the `active_preset` anchor (or "none"); `focus` is a line from
      `## Current focus` (or left blank); `review` is the count in `## Needs review`
      (or "clear"); `last` is the newest `log.md` entry, given relatively. It does not
      invent state the brain does not hold.
...
- [ ] **British English, genuine voice.** It reads as {{OWNER}}'s own hand, not generic
      machine prose.
- [ ] **No machine tells.** No dash used as a connective tic, no ideas padded into tidy
      triplets, no templated scaffolding or filler.
...
- **Machine tells in a deliverable.** Dash connectors, rule-of-three padding,
