...
**recency** (is it current for a claim that changes over time?). When unsure,
**rate down**, preferring precision over optimism. These cases are the acceptance
bar: an agent that rates them differently is mis-calibrated.
...
| 6 | A reputable outlet, but the specific claim is unverifiable and uncorroborated | **medium** | Authority present, corroboration absent, so rate down from high. |
| 7 | A primary regulatory filing, current, but dense/ambiguous on the exact point | **medium** | Primary and current, but the *specific* claim is not clearly supported, so rate down. |
