...
The acceptance checklist the **instantiation kit** must pass before it is trusted (the
S19 kit — [`SETUP.md`](../../../SETUP.md), [`ARCHITECTURE.md` §1](../../../ARCHITECTURE.md)).
Two halves: **Part A** — the [`instantiate`](../tools/instantiate) tool's deterministic
guarantees; **Part B** — the [`setup`](../skills/setup/SKILL.md) skill's judgement
responsibilities. The governing rule throughout: **the instance is created OUTSIDE the
engine and never committed to it**, and **Model A topology** (the instance preserves the
engine's nesting so every instrument→authority-doc link resolves untouched). This restates
the authority as a *test*, not a new authority. If any item fails, the kit is not correct.
...
- [ ] **Model-A topology** — the instance is `<target>/` with the authority docs +
      ...
      `<target>/vault/` (the stamped `vault-template/` contents). A sampled instrument→doc
      link (e.g. a skill's `../../../../OPERATIONS-KERNEL.md`) **resolves to a real file** in
      the instance — proving the nesting was preserved.
- [ ] **Operational stamping, docs left alone** — `{{OWNER}}`/`{{COMPANY}}` are stamped in
      ...
      authority docs at the instance root are copied **UNSTAMPED** (generic instruction —
...
- [ ] **Excludes honoured** — `.git/`, `.obsidian/`, `substrate/store/`, `*.shard`/`*.vec`/
      model artefacts, `.env` (not `.env.example`), `__pycache__/` are **not** copied — no
      stale runtime state or secret rides into the instance.
- [ ] **Instance `.gitignore` is instance-appropriate** — generated, NOT the engine's: it
      keeps `vault/` (the owner's brain, versionable privately) but excludes secrets +
...
- [ ] **Data-boundary guard (the load-bearing one)** — a target **inside the engine** (the
      engine root, `vault-template`, or any subpath) → **exit 2, nothing written**. A
      **non-empty** target → exit 1 (re-instantiation is a deliberate delete-and-recreate,
      never a silent overwrite). A `--preset` naming a preset that doesn't exist → exit 1
      **before any copy** (no half-built instance left behind). The engine is only ever READ.
- [ ] **Deterministic & least-privilege** — same `(engine content, args, --now)` → the same
      instance; `--now` makes seeded timestamps reproducible; `--dry-run` prints the plan and
      writes nothing; **no network, no credentials**. Exit 0 success, 1 recoverable refusal,
      2 hard failure; errors → stderr.
...
- [ ] **Owns the machine-specific wiring only** — embedding adapter (local pull+pin / API
      key via `.env`), schedules + `SessionEnd` hook, watchlist, `.env` from `.env.example`,
      model-bindings — the parts a tool may not do (network/credentials/cron).
...
- **Stamping the authority docs** — rewriting `{{OWNER}}` in ARCHITECTURE/etc. so the
...
- **A secret or stale store copied in** — `.env`, a `*.shard`/`*.vec`, `substrate/store/`,
