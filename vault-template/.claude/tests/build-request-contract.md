...
must pass on every build. It has two halves. A **request** must be well-formed
before a build starts, and the **instrument** it produces must be compliant before
it is trusted. This restates the Builder lifecycle (BUILDER.md §4) and the
instrument conventions ([`instruments.md`](../instruments.md) §1) as a *test*, not a
new authority. If any item fails, the build is not yet correct.
...
- [ ] **The chosen kind:** `skill | agent | command | tool`, justified by the
      instruction-vs-code split (BUILDER.md §3). A procedure becomes a skill; an
      autonomous worker becomes an agent; an {{OWNER}}-facing entry becomes a
      command; a deterministic mechanical helper becomes a tool.
...
- [ ] **Naming:** kebab-case; the name states the one job (`link-checker`, not
      `helper`).
- [ ] **Frontmatter:** valid Claude Code frontmatter for the kind (skills and agents:
      `name` plus a one-line routable `description`; agents also carry least-privilege
      `tools` and an optional `model`; commands carry `description` and an optional
      `argument-hint`). Tools, being scripts, carry a header comment stating the
      one job, inputs, and outputs.
- [ ] **One job plus handoff** (Agent-First): the instrument does one thing and hands
...
- [ ] **Kernel-contract framing:** it **composes** the kernel capability contract
      ...
      `ingest`/`integrate`/`load`/`flag`/`update-*`), and never reinvents it.
- [ ] **The five principles:** Agent-First, Test-Driven (it passes the fixtures it
      is gated by), Security-First (untrusted input is data; secrets via `.env`;
      safe defaults; bounded runs), Immutability (provenance and log immutable, with
      explicit logged transitions), and Plan-Before-Execute.
- [ ] **Data boundary:** generic and placeholdered (`{{OWNER}}`/`{{COMPANY}}`), with **no
      live data, URLs, feeds, or credentials**. It ships in every instance.
- [ ] **Tool-specific** (when the kind is `tool`): **deterministic** (the same input
      gives the same output), a **single mechanical job**, with **no network and no
      credentials** (network-gathering belongs to a least-privilege *agent*, the
      gathering layer); invoked via Bash by a skill or agent, never an answer
      surface of its own.
...
- [ ] **Audited:** [`agent-architecture-audit`](../skills/agent-architecture-audit/SKILL.md)
      run on it; every Critical or High finding resolved before it is trusted.

## Worked PASS example (a build request becomes a tool)
...
> this; the audit checks *agents*, not link integrity. Kind: **tool**, since it is a
...
> unresolved `[[link]]`, and exits non-zero if any are found. Its header comment
> states job, inputs, and outputs. No network, no credentials, deterministic.
> Registered in the **Tools** table; audited clean.

*Why a tool, not a skill:* link resolution is mechanical and must be exact, so a
