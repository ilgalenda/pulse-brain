...
— a procedure invoked by a command or an agent. A thin **wrapper over the global
...
check ([`instruments.md`](../../instruments.md) first — extend an existing skill
...
- **Single job + handoff** (Agent-First) — a skill is one procedure; it composes
  ...
  specialist work off, never reinvents a primitive.
