...
A deliberate or scheduled audit of the **platform**, not the knowledge — distinct
from a Thinking sweep ([THINKING §4](../../../../THINKING.md), which keeps health
metrics out and assigns them here). Use it to find orphans, stale edges, unresolved
contradictions, confabulation surfaces, hub/duplicate defects, ontology-promotion
candidates, and recurring capability gaps — then route each.
...
1. **Audit.** Run the mechanical signals via the
   ...
   exact-duplicate titles · open `edge/contradicts` tags — deterministic, cite its
   output). Reason out the judgement signals (hub overload and weak connectivity;
   semantic duplicates come from the S17 substrate's duplicate finder as
   threshold-gated **candidates** once calibrated (S19) — until then reason them out;
...
4. **Route — drive the owning hand** ([§4](../../../../SELF-DEVELOPMENT.md) routing
   ...
   - unsourced-and-unmarked → a **research prompt** onto the `edge/inference` queue
     ([`research`](../research/SKILL.md) / `research-agent`, the S10/S11 loop);
...
5. **For the two owned decisions — prepare a proposal to the gate** (§6, tier 3).
   ...
   **propose build-complete, never apply** (growth-proposal-contract Part B): draft
   ...
   change), validate it against the existing vocabulary / surrounding docs, evidence
   the recurrence, and queue it as a **Pulse decision** via the existing two-part flag
   (page callout + `core.md` "Needs review", kernel §10). The engine doc stays
...
6. **Record.** Stamp the pass — what it found, what it dispatched, what it queued —
   ...
   kernel to write, never writing `log.md`/`core.md` itself.
...
- **Never silently change shared meaning.** Ontology and architecture edits are
  ratify-gated (§6); propose, queue, log — never apply unasked.
...
- **Compose, add nothing.** No new kernel primitive, no new flag — the existing
