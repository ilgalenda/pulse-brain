---
name: monitor
description: Run one monitoring beat for the brain, checking the instance watchlist for new or changed material since the last run and filing dynamic-notes for what's genuinely new. Use for scheduled vigilance ("check the watchlist", "any updates on the things we monitor"). The canonical procedure /monitor and monitor-agent compose.
---
...
4. For each genuine change, **run the `research` capture path**: assess confidence
   ...
   discipline rather than restating it, per Agent-First.)
...
- **Immutability**: new or changed material is a *new* note that `supersedes` the
  prior. The old one is kept for provenance, not re-surfaced (Dynamic §3).

## Output

A one-line report per beat: sources checked, what was new, what was filed or fed,
