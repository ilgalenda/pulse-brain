---
description: Run a self-development audit pass now — measure the brain's own graph health and capability gaps, route each weakness to the layer that fixes it, and review any proposals queued for ratification.
argument-hint: [optional pillar or domain slice to scope the audit]
---

Run a **self-development audit pass** (the S13 capability — see
[`SELF-DEVELOPMENT.md`](../../../SELF-DEVELOPMENT.md)).

This is {{OWNER}}'s entry to the growth meta-loop; the *capability* belongs to the
Self-Development layer, this just invokes it (the same pass S11 schedules unattended,
[`scheduling.md`](../scheduling.md)). It audits the **platform**, not the knowledge —
distinct from [`/sweep`](sweep.md), which reasons over content
...
- **routes** each finding to the owning layer (§4) — Thinking, the kernel's
  edge-work, the `edge/inference` research queue, or a Canon split/merge candidate —
  and **drives** it; it never performs the fix itself;
