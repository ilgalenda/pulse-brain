...
Refresh the **semantic vector index** over the canon wiki (the Model & Retrieval
...
- **`local`** — the on-device model (zero-API-config); requires the S19 model pull +
  checksum (`MODEL-RETRIEVAL.md` §3.6).
- **`api`** — opt-in; requires the embedding key in `.env`.

Indexing is **incremental by content-hash**: unchanged pages are skipped, changed/new
pages are re-embedded, and pages removed from canon have their shards pruned. The store is
written single-writer-safe (per-page shards + atomic rebuild), so a concurrent tab cannot
tear it ([DEVELOPMENT.md §6](../../../DEVELOPMENT.md) lane note). The store is **instance
runtime state — gitignored, never committed** (§5).

Report back: pages indexed / skipped / removed, the adapter used, and any page that
failed to embed. **Deferred to S19** ([`scheduling.md`](../scheduling.md)): a reindex
cadence, threshold calibration, and turning semantic re-ranking *on* in read-routing —
until then `/reindex` maintains the index but the kernel's read-routing stays graph-first
with semantic similarity as the reserved, not-yet-active tie-breaker.
