...
It is an **instruction** layer (in the Brain-OS instruction-vs-code map, this is the graph *model* the LLM follows). The *code substrate* that runs the graph at scale, covering embeddings, semantic edges, automated traversal and metrics, and vector retrieval, is the **Model & Retrieval Substrate** ([`MODEL-RETRIEVAL.md`](MODEL-RETRIEVAL.md)). This document defines what the graph **is**; the substrate builds the machinery (§9).
...
A **node** is one **atomic markdown file** (one idea, one file, per Brain-OS Law 2), identified by its stable `id` (set once, never changed; the durable handle for cross-references, per [`TAXONOMY.md`](TAXONOMY.md)). The node kinds:
...
**Not nodes: infrastructure.** The brain artefacts (`index.md`, `log.md`, `memory/core.md`, and the per-pillar `_index.md` edge-maps) are *about* the graph; they are not knowledge nodes in it. They orient, serialise, and remember; they are not traversed as content.
...
**Representation: no new required field.** These already have homes. `supersedes` is the supersede body-note plus a `related` link to the replacement; `contradicts` is the `[!contradiction]` callout (Canon §4); `corroborates` lives in the corroborating source's extracted points and `feeds`. This document **names** them as edge types and unifies their meaning; where an edge-map needs to show one, it is surfaced as a **tag** (for example `edge/contradicts`), not a new frontmatter schema field. The graph stays expressible in the existing structure. **The tag sits on the node carrying the open relation and clears when it resolves**: `edge/supersedes` on the superseded (older) node, `edge/contradicts` on a node in unresolved conflict. So an open tag in the edge-map is itself the unresolved signal (the same convention `edge/inference` follows, below). Direction in the table above is the relation's semantics, not where the tag lives.
...
The edge vocabulary is **open**. A rich ontology imposed up front ossifies; richness should **emerge as the brain grows**. But ungoverned emergence drifts into inconsistency, with one relation expressed five ways. So new relation types are **promoted deliberately**:

1. **Provisional**: a relation is first expressed in prose or as an annotated `related` link, not yet a named type.
...
Promotion is reviewed through the **graph-health / Self-Development loop**. The ontology grows with the brain, seeded small and extended on evidence.

## 4. Edge representation and the edge-map
...
- **Anchor** on the nodes the task concerns (the entities and pages it names).
- **Walk** outward over the edge-map `links`, with **bounded depth**, **distance decay** (nearer nodes weigh more), and **hub weighting** (highly-connected nodes are strong signals).
- **Tie-break** on domain, tags, and recency. Semantic similarity is added by the retrieval substrate as a **re-rank** of the surfaced candidates, never as a recall path that adds graph-distant pages ([`MODEL-RETRIEVAL.md`](MODEL-RETRIEVAL.md) §1; turned on once read-routing integration lands).
...
The graph's condition is itself a signal. This document **defines** these; **Self-Development monitors and acts on** them:
...
- **Unsourced-and-unmarked claim**: a canon claim with no cited `[[source]]` *and* no `[!inference]` marker, which is the confabulation surface. Surfaced by the Thinking layer's grounding re-trace ([`THINKING.md`](THINKING.md) §6.2) so a guess that escaped its marker is caught on a later pass, not left to read as fact.
...
- **Prune** dead edges: re-point or drop links to superseded or merged nodes.
...
This document is the authority; these are the consumers:

- **Operations Kernel**: applies traversal (§7-read), maintains the edge-map serialisation (§8), runs edge-work in integration (§13).
- **Context Engine**: Canon §7 (page-level link discipline), Dynamic/Inbox graph-participation (source nodes and their edges).
- **Thinking**: proposes new edges and synthesis across the graph.
- **Self-Development**: monitors graph-health (§6) and reviews ontology promotions (§3.3).

## 9. Seam to the Model & Retrieval Substrate

This document is **instruction-tier**: the graph model the LLM follows by reading and writing markdown. The Model & Retrieval Substrate is **code**: the substrate for running this model at scale. Its full *remit* spans embeddings and **semantic (similarity) edges**, **automated traversal and graph-health metrics**, and **vector retrieval** layered over the explicit graph, though the current build delivers only part of it (see below). This document defines what the graph is and how it should grow and be read; the substrate builds the machinery to do it programmatically. The model comes first so the machinery has something correct to implement.

**Already built** ([`MODEL-RETRIEVAL.md`](MODEL-RETRIEVAL.md)): the deterministic shell, namely the embedding/store substrate, vector **re-ranking**, and the semantic-duplicate and edge-candidate finders. **Not yet part of the build**: automated graph traversal and graph-health metrics. Traversal stays the instruction-tier read model above, and the mechanical health signals stay `graph-health-scan`'s; only the *semantic*-duplicate signal is the substrate's.

Two boundaries the machinery honours. Vector retrieval is **re-ranking only**: it re-orders the walk's candidates, never adds graph-distant pages, so the graph stays load-bearing. And embedding-derived edges and semantic-duplicate findings are **candidates** the Thinking layer proposes, never auto-written. Threshold calibration and read-routing integration come later.
